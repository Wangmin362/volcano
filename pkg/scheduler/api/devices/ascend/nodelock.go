...
func SetNodeLock(client kubernetes.Interface, nodeName string, lockname string, pods *corev1.Pod) error {
	lock.Lock()
	defer lock.Unlock()
...
func ReleaseNodeLock(client kubernetes.Interface, nodeName string, lockname string, pod *corev1.Pod, timeout bool) error {
	lock.Lock()
	defer lock.Unlock()
	ctx := context.Background()
	node, err := client.CoreV1().Nodes().Get(ctx, nodeName, metav1.GetOptions{})
...
func LockNode(client kubernetes.Interface, nodeName string, lockname string, pods *corev1.Pod) error {
	ctx := context.Background()
	node, err := client.CoreV1().Nodes().Get(ctx, nodeName, metav1.GetOptions{})
	if err != nil {
		return err
	...
	if _, ok := node.Annotations[NodeLockKey]; !ok {
		return SetNodeLock(client, nodeName, lockname, pods)
	}
	...
	if time.Since(lockTime) > NodeLockTimeout {
		...
		err = ReleaseNodeLock(client, nodeName, lockname, pods, true)
		if err != nil {
